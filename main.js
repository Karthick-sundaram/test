...
const VIDEO_URL = 'single-video.mp4';
...
cacheBtn.addEventListener('click', async function () {
  ...
  try {
    ...
    if (response.ok) {
      ...
      cachedStatus.textContent = 'Low network. Partial response, Cannot cache!';
      throw new Error("Partial response detected, cannot cache.");
    } else {
      hideProgressAnimation();
